...
By default, then `print` function will only display 20 rows.  Change the default to
another value using the following command:
...
we will truncate the output to 5 rows.
...
You can also use or `transform()` `within()` to rename a column:
...
Note that the new column will appear at the end of the dataset. To rename the
column in-place do the following instead:


```r
## Not run: names(flights)[match("tailnum", names(flights))] <- "tail_num"
```

(Admittedly, none of these approaches are as convenient as `dplyr::rename()`.)
